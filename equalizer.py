...
def plan_evictions(
    ...
    for node in nodes:
        ...
        if len(candidates) < overload:
            ...
            print(
                f"[warn] Node {node} is {overload} pods over target "
                f"but only {len(candidates)} are safe to evict. "
                f"{shortage} pods will remain imbalanced.",
                file=sys.stderr,
            )
...
def warn_on_unlisted_nodes(pods_by_node: Dict[str, Sequence], nodes: Sequence[str]) -> None:
    ...
    if unknown:
        joined = ", ".join(unknown)
        print(
            "[warn] Found pods scheduled on nodes outside the selected pool: "
...
def execute_plan(
    ...
    for node, pod in plan:
        if max_evictions is not None and performed >= max_evictions:
            print("Reached --max-evictions limit, stopping.", file=sys.stderr)
            break
        if dry_run:
            continue
        ...
        try:
            ...
            print(
                f"[error] Failed to evict {pod.metadata.namespace}/{pod.metadata.name}: {exc}",
                file=sys.stderr,
            )
    return performed
...
def main():
    ...
    if evicted:
        print(f"Successfully issued {evicted} eviction request(s).")
    else:
        print("No eviction requests were issued.")
    return 0
